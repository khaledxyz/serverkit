...
export const PanelProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    const [state, dispatch] = useReducer(panelReducer, getDefaultState(), () => getStoredPanelState());
    ...
    const toggleBottom = useCallback(() => dispatch({ type: 'TOGGLE_BOTTOM' }), []);
...
export const usePanelStore = () => {
    ...
    if (!ctx) throw new Error("usePanelStore must be used within a PanelProvider");
    return ctx;
};
